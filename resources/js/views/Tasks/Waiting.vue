<template>
  <div class="p-4 lg:p-8">
    <div class="max-w-4xl mx-auto">
      <div class="mb-6">
        <h1 class="text-xl lg:text-2xl font-semibold text-gray-900 dark:text-white">Ожидание</h1>
      </div>
      ...
      <template v-else>
        ...
        <div v-else class="text-center py-12">
          <div class="max-w-md mx-auto">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Нет задач в ожидании</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-6 leading-relaxed">
              Список ожидания — это задачи, делегированные другим людям, или те,
              что зависят от внешних событий. Записывайте, от кого и чего вы ждёте.
              Регулярно проверяйте этот список, чтобы ничего не потерялось.
            </p>
            <button
              @click="handleAddTask"
...
const handleToggleComplete = async (task) => {
  try {
    if (task.status === 'completed') {
      await tasksStore.updateTask(task.id, { status: 'waiting' })
    } else {
      await tasksStore.completeTask(task.id)
    }
  } catch (error) {
